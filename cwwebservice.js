		...
		if(isInputValid(sSectionLabel))
		{
			...
			if(oSection)
			{
				if(oContainer.children.length==0)
				{
					...
					//oParentListItemNode.setAttribute("jumpcode",sMainSectionId);
					...
					oParentListItemNode.setAttribute("onclick","gotoSection(this)");
					...
					//oParentListItemNode.setAttribute("onclick","test5(this)");
					//addElementAttribute(sHTMLElement,sAttributeName,sAttributeValue)
				}
								...
								for(var j=aOuterSections.length;j>=0;j--)
								{
									...
									if(oOuterSection && oHTMLParentElement)
									{
										...
										if(sCtypeVal=="NOTECONTROL"||sCtypeVal=="SUBNOTECONTROL"||sCtypeVal=="EXPANDCOLLAPSE"||sCtypeVal=="SUBNOTECONTROLGROUP"||sCtypeVal=="EXPANDCOLLAPSECOMPANY3RDYEAR")
										{
											...
											oChildListItemNode.setAttribute("onclick","gotoSection(this)");
											...
											oChildListItemNode.setAttribute("style","background-color:#F5DEB3");
										}
...
function getParaData(oDoc,oSubSection)
{
	try{
		...
		sStr = "<li component='inputtextsection' id='"+sSubSection+"' objecttype='section' jumpcode='"+sSubSection+"' onclick='gotoSection(this);' guid='"+sParaGuid+"' style='background-color:#CCFFCC;color:"+sColor+";' ctype='"+sCtypeVal+"' title='Component: Paragraph\nParagraph index: "+oPara.index+"'>"+sParaText+"</li>";	
	...
	try{		
		...
		switch (sTableType)
		{
			...
			case "6N2":
				...
				var aDescCol = getColWithProp(CCOLTYPE,DESC_COL,oTable);//oCustTable.getColWithProp(CCOLTYPE,DESC_COL);
				var aMapCol = getColWithProp(CCOLTYPE,MAP_COL,oTable);//oCustTable.getColWithProp(CCOLTYPE,MAP_COL);
				//var aNoteRefCol = oCustTable.getColWithProp(NOTEREF_COL);
				var aCalcCol = getColWithProp(CCOLTYPE,CALC_COL);//oCustTable.getColWithProp(CALC_COL);
